...
class DrawingApp:
    ...
    def new_image(self):
        if messagebox.askokcancel("Новый холст", "Вы уверены, что хотите создать новый холст? Несохраненные данные пропадут навсегда."): 
    ...
    def about(self):
        messagebox.showinfo("О программе", "[ENG]\nMaxim paint version-build16 demonstration\nprogramm created by maxim bashmak\n\n[RUS]\nMaxim paint версии-билд 16 демонстрационный\nпрограмма создана Максимом Башмачком\n\n[INFO]\nСтандартная рисовалка в которой можно побаловатся или порисовать, так-же можно отредактировать изображение")
